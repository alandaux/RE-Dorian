## Spatial Clustering Analysis for BirdDay Twitter Analysis
# Code by Joseph Holler (2021) and Casey Lilley (2019), edited by Arielle Landau 2021
...
birding = readRDS(here("data","derived","private","birding.RDS"))
mayTweets = readRDS(here("data","derived","private","mayTweets.RDS"))
...
birding_by_county = birding_sf %>%
  st_drop_geometry() %>%   # drop geometry / make simple table
  group_by(GEOID) %>%      # group by county using GEOID
  summarise(birding = n())  # count # of tweets

counties = counties %>%
  left_join(birding_by_county, by="GEOID") %>% # join count of tweets to counties
  mutate(birding = replace_na(birding,0))       # replace nulls with 0's

rm(birding_by_county)

# Repeat the workflow above for tweets in May

may_by_county = mayTweets %>% 
...
counties = counties %>%
  mutate(birdrate = birding / POP * 10000) %>%  # dorrate is tweets per 10,000
  mutate(ntdi = (birding - may) / (birding + may)) %>%  # normalized tweet diff
...
counties$locG = as.vector(localG(counties$birdrate, listw = dwm, 
                                 zero.policy = TRUE))
...
ggplot() +
  ...
  labs(title = "Clusters of Twitter Activity around BirdDay 2021")+
