# Analyze Birding Twitter data around BirdDay, May 4, 2021, by Joseph Holler, 2019 and edited by Arielle Landau, 2021
...
birdingText = birding %>% select(text) %>% plain_tweets()

# parse out words from tweet text
birdingWords = birdingText %>% unnest_tokens(word, text)
...
stop_words = stop_words %>%
  ...
  add_row(word="birding",lexicon = "Search") %>%
  add_row(word="birdday",lexicon = "Search") %>%
  add_row(word="birds",lexicon = "Search") %>%
  add_row(word="migration",lexicon = "Search") %>%
  add_row(word="spring",lexicon = "Search") %>%
  add_row(word="bird",lexicon = "Search")

#delete stop words from dorianWords with an anti_join
birdingWords =  birdingWords %>% anti_join(stop_words)
...
birdingWordPairs %>%
  filter(n >= 10 & !is.na(word1) & !is.na(word2)) %>%
  graph_from_data_frame() %>%
  ggraph(layout = "fr") +
  geom_edge_link(aes(edge_alpha = n, edge_width = n)) +
  geom_node_point(color = "darkslategray4", size = 3) +
  geom_node_text(aes(label = name), vjust = 1.8, size = 3) +
  labs(title = "Word Network of Bird Tweets around BirdDay",
       x = "", y = "") +
  theme_void()
...
ggplot() +
  ...
  geom_point(data = birding, aes(x=lng,y=lat),
             colour = 'purple', alpha = .2) +
  labs(title = "Tweet Locations Around BirdDay")+
...
birdingsql <- select(birding,c("user_id","status_id","text","lat","lng"),
                    starts_with("place"))

#write data to the database
dbWriteTable(con,'birding',birdingsql, overwrite=TRUE)

# try also writing the november tweet data to the database! Add code below:
mayBirdingsql <- select(mayTweets, c("user_id","status_id","text","lat","lng"), starts_with("place"))
dbWriteTable(con, 'mayTweets',mayBirdingsql, overwrite=TRUE)
